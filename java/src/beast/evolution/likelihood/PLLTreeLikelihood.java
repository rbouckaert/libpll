...
/**
 * PLLTreeLikelihoodModel - implements a Likelihood Function for sequences on a tree.
 *
...
@Description("Uses PLL library to calculate Tree likelihood")
public class PLLTreeLikelihood extends TreeLikelihood {

    // This property is a comma-delimited list of resource numbers (0 == CPU) to
    // allocate each PLL instance to. If less than the number of instances then
    // will wrap around.
...
//    private double [] storedCurrentCategoryRates;
    ...
    private boolean initialize() {
        ...
        if (resourceOrder.size() > 0) {
            ...
            if (resourceList[0] > 0) {
                preferenceFlags |= PLLFlag.PROCESSOR_GPU.getMask(); // Add preference weight against CPU
            }
        }
        ...
        if (this.rescalingScheme == PartialsRescalingScheme.AUTO) {
            preferenceFlags |= PLLFlag.SCALING_AUTO.getMask();
            useAutoScaling = true;
        } else {
//                preferenceFlags |= BeagleFlag.SCALING_MANUAL.getMask();
        ...
        InstanceDetails instanceDetails = pll.getDetails();
...
//            }
        ...
        for (int i = 0; i < tipCount; i++) {
            ...
            if (m_bUseAmbiguities || m_bUseTipLikelihoods) {
                setPartials(pll, i, taxon);
            } else {
                setStates(pll, i, taxon);
            }
        }
        ...
        if (this.rescalingScheme == PartialsRescalingScheme.AUTO &&
                ...
                (resourceDetails.getFlags() & PLLFlag.SCALING_AUTO.getMask()) == 0) {
        ...
        pll.setCategoryRates(categoryRates);
...
//        updateRestrictedNodePartials = true;
    ...
    public double calculateLogP() {
        ...
        if (operations == null) {
            operations = new int[1][internalNodeCount * PLL.OPERATION_TUPLE_SIZE];
...
//                pll.accumulateScaleFactors(scaleBufferIndices, internalNodeCount, PLL.NONE);
            ...
            for (int i = 0; i < categoryWeights.length; i++) {
            	if (categoryWeights[i] != currentCategoryWeights[i]) {
                    pll.setCategoryWeights(0, categoryWeights);
            ...
            for (int i = 0; i < frequencies.length; i++) {
            	if (frequencies[i] != currentFreqs[i]) {
                    pll.setStateFrequencies(0, frequencies);
...
//    }
    ...
    protected void setPartials(int number, double[] partials) {
        pll.setPartials(partialBufferHelper.getOffsetIndex(number), partials);
    }
...
//            }
        ...
        if (!node.isLeaf()) {
            ...
            if (update1 != Tree.IS_CLEAN || update2 != Tree.IS_CLEAN) {
                ...
                int x = operationCount[operationListCount] * PLL.OPERATION_TUPLE_SIZE;
